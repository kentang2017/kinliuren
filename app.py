...
with pan:
    ...
    ltext1 = liuren_day
    ltext2 = liuren_hour
    mchin = day_chin(qgz[4][1], weekday(y, m, d))
    ...
    p = "\n地禽︰"+ dict(zip(list("子丑寅卯辰巳午未申酉戌亥"),new_list(chin_list, mchin)[0:12])).get(qgz[3][1]) + "(主) vs  天禽︰" + dict(zip(list("子丑寅卯辰巳午未申酉戌亥"),new_list(chin_list, mchin)[0:12])).get(richp) + "(客)"
