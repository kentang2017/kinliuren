...
class Liuren():
    ...
    def shehai(self):
        ...
        if d == 1 and self.find_sike_relations()[7][0] != "試涉害":
            ...
            sike = self.find_sike_relations()[1]
            if self.find_sike_relations()[2].count("被尅") == 1 and self.find_sike_relations()[2].count("尅") == 1:
                if self.find_sike_relations()[5] == "尅" or "被尅":
    ...
    def sha(self, jq, g, z):
        ...
        season = list("春夏秋冬")
        return {"天城":"申", "天吏":"寅",
                "皇書":dict(zip(season, list("寅巳申亥"))).get(s), 
                "天喜":dict(zip(season, list("戌丑辰未"))).get(s), 
                "天耳":dict(zip(season, list("申寅巳亥"))).get(s), 
                "戲神":dict(zip(season, list("巳子酉辰"))).get(s), 
                "遊神":dict(zip(season, list("丑子亥戌"))).get(s), 
                "天車":dict(zip(season, list("巳辰未酉"))).get(s), 
                "月馬":self.moonhorse(),
...
if __name__ == '__main__':
    ...
    print(d +"     " + h)
    print(Liuren(j, "六", d, h).shehai2())
    print("    ")
    print(Liuren(j, "六", d, h).shehai())
